...
namespace market
{
    public class itemInfo
    {
        ...
        List<itemInfo> item_list = default;
        ...
        public void itemMarket()
        {
            ...
            itemInfo waterbottle = new itemInfo();
            waterbottle.Initem("김재현의 물병", 100);
            itemInfo glasses = new itemInfo();
            glasses.Initem("박시연의 알없는 안경", 15000);
            itemInfo macbook = new itemInfo();
            macbook.Initem("배경택의 맥북", 39800);
            itemInfo pouch = new itemInfo();
            pouch.Initem("이미정의 곰돌이 파우치", 100000);
            itemInfo pin = new itemInfo();
            pin.Initem("박준오의 머리핀", 8000);
            itemInfo pants = new itemInfo();
            pants.Initem("유준호의 찢어진바지", 80000);
            itemInfo coffee = new itemInfo();
            coffee.Initem("교수님의 아이스아메리카노", 3000);
            itemInfo mandarin = new itemInfo();
            mandarin.Initem("신성창의 제주감귤", 200);
            itemInfo IQOS = new itemInfo();
            IQOS.Initem("이경민의 부숴진 아이코스", 5000);
            itemInfo lighter = new itemInfo();
            lighter.Initem("박정근의 세상을 멸망시킬 라이터", 200);


            item_list = new List<itemInfo>();
